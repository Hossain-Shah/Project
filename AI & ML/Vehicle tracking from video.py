...
car_cascade = cv2.CascadeClassifier('D:/Anaconda/opencv/build/etc/haarcascades/haarcascade_car.xml') #Path to cars.xml model
...
while True:
    ...
    for (x, y, w, h) in cars:
        if (x >= coord[0][0] and y == coord[0][1]):
            cv2.line(img, (coord[0][0], coord[0][1]), (coord[1][0], coord[1][1]), (0, 255, 0),
